...
<script type="text/javascript">
        ...
        return `
            <div style="margin-bottom: 20px; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
                ...
                <ul style="list-style-type: none; padding-left: 10px; margin-top: 5px;">
                    ${{Array.from(directCalls).map(node => `
                        <li>
                            ${{node}}
                ...
                <ul style="list-style-type: none; padding-left: 10px; margin-top: 5px;">
                    ${{Array.from(indirectCalls).map(node => `
                        <li>
                            ${{node}}
                            ${{createJobsHTML(getNodeJobs(node))}}
                        </li>
                ...
                <ul style="list-style-type: none; padding-left: 10px; margin-top: 5px;">
                    ${{Array.from(indirectCalledBy).map(node => `
                        <li>
                            ${{node}}
                            ${{createJobsHTML(getNodeJobs(node))}}
                        </li>
    ...
    // Function to traverse and find dependencies
    function getDependencies(scriptId) {{
        ...
        calledBy.forEach(function(directCaller) {{
            Object.keys(scriptsData).forEach(otherScript => {{
                if (scriptsData[otherScript].calls.includes(directCaller) && otherScript !== scriptId && !calledBy.has(otherScript)) {{
    ...
    function addScript(scriptId) {{
        ...
        // Add the selected script itself
        if (!addedNodes.has(scriptId)) {{
            network.body.data.nodes.update([{{
                id: scriptId, 
                label: scriptId,
        ...
        // Add to the selected nodes list in the UI
        let selectedList = document.getElementById('selectedNodesList');
        let listItem = document.createElement('li');
        listItem.id = `selected-${{scriptId}}`;
        listItem.style.marginBottom = '5px';
        listItem.innerHTML = `
            ${{scriptId}} <button onclick="removeScript('${{scriptId}}')" style="margin-left: 10px; padding: 2px 5px;">Remove</button>
        `;
